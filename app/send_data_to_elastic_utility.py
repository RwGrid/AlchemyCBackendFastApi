...
def process_episode_to_postgres(sent_json):
    try:

        record = dict(sent_json)
        current_date_time = ddt.datetime.now()
        ...
        try:
            os.mkdir('inserted_postgres_objects')
        except:
            pass
        try:
            os.mkdir('inserted_postgres_objects' + '/' + str(dt0))
        except:
            pass
        # Writing to sample.json
        with open('inserted_postgres_objects' + '/' + str(dt0) + '/' + str(tm0) + '.json', "w") as outfile:
        ...
        episode_inform = {}
        episode_inform["data_insertion_date"] = combined0
        episode_inform["day"] = record['formDay'].strip()
        date_of_episode = parse(record['formEpiTime'].strip(), fuzzy=True).strftime("%Y-%m-%d")
        time_of_episode = parse(record['formEpiTime'].strip(), fuzzy=True).strftime("%H:%M:%S")
        episode_inform["episode_date"] = str(date_of_episode).strip()
        episode_inform["episode_time"] = str(time_of_episode).strip()
        if 'formEditors' in record:
            if record.get('formEditors', None) is not None:

                if len(record['formEditors']) > 0:
                    episode_inform['producers_team'] = '|'.join(record['formEditors'])
        if 'formGraphic' in record:
            if record.get('formGraphic', None) is not None:

                if len(record['formGraphic']) > 0:
                    episode_inform['technical_team'] = '|'.join(record['formGraphic'])

        episode_inform["episode_type"] = str(record['formEpisodeType']).strip()

        if isinstance(date_of_episode, str):
            date_of_epi = date_of_episode.strip()

            date_of_epi = parse(date_of_epi, fuzzy=True)

        if isinstance(time_of_episode, str):
            time = time_of_episode.strip()

            dt = parse(time, fuzzy=True)
            exacttime = dt_lib.datetime.strptime(time, '%H:%M:%S')

        # dt = parse(record['Date'], fuzzy=True)
        year = date_of_epi.year
        month = date_of_epi.month
        day = date_of_epi.day
        hour = exacttime.hour
        ...
        combined_date_time_of_epi = dtm.isoformat(dtm.combine(dt, tm))

        episode_inform["date_time_of_epi"] = combined_date_time_of_epi
        episode_inform["date_time"] = record['formProgramName']
        dict2["رايط المقطع"] = ''
        dict2["عنوان المقطع"] = ''
